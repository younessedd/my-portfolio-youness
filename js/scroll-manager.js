...
class ScrollManager {
     ...
     */
    lockScroll(popupId = 'default') {
        // Only lock scroll for mobile menu, not for popups since they now scroll with page
        if (popupId !== 'mobile-menu') {
            console.log(`📱 Popup ${popupId} now scrolls with page - no scroll locking needed`);
            return;
        }
    ...
    setupMobileTouchHandling() {
        // Since popups now scroll with the page, we only need to handle mobile menu scroll locking
        console.log('📱 Mobile touch handling setup complete - popups now scroll with page');

        // Note: Touch event handling is now simplified since popups are part of normal page flow
        // Only the mobile menu needs special touch handling when it's open
    }
