/**
 * IoT Projects Category Swiper
 * Single Swiper with category buttons and autoplay
 * Uses same card styling as Web Projects
 */

(function() {
    'use strict';
    
    const categories = ['InternetofThings', 'others'];
    ...
    function setup() {
        if (typeof window.iotProjectsData === 'undefined') {
    ...
    function collectAllProjects() {
        ...
        categories.forEach(cat => {
            ...
            const projects = window.iotProjectsData[cat] || [];
            projects.forEach(p => {
    ...
    function setupButtons() {
        document.querySelectorAll('.iot-category-btn').forEach(btn => {
    ...
    function createSlide(project) {
        ...
        const img = project.images && project.images[0] ? project.images[0] : 'images/iot/placeholder.webp';
        
        // Use EXACT SAME card structure as Web Projects for identical styling
        slide.innerHTML = `
            <div class="web-card">
    ...
    function initSwiper() {
        const wrapper = document.getElementById('iot-projects-cards');
        
        allProjects.forEach(project => {
            wrapper.appendChild(createSlide(project));
        });
        
        swiper = new Swiper('#iot-projects-swiper', {
            loop: false,
            speed: 700,
            spaceBetween: 30,
            autoplay: {
